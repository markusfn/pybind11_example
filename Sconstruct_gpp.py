...
# Ensure the temporary directory exists before g++ tries to use it
os.makedirs(USER_TEMP_DIR_WIN, exist_ok=True)
print(f"Using temporary directory for g++: {USER_TEMP_DIR_WIN}")
...
env = Environment(
    ...
    ENV={
        ...
        'TMPDIR': USER_TEMP_DIR_WIN, # <--- CRITICAL: Force g++ to use this dir for its temps
...
env['CXX'] = os.path.join(MSYS2_UCRT64_ROOT, 'bin', 'g++')
env['CC'] = os.path.join(MSYS2_UCRT64_ROOT, 'bin', 'gcc')
env['LINK'] = os.path.join(MSYS2_UCRT64_ROOT, 'bin', 'g++')
...
print(f"Building {MODULE_NAME}.pyd with G++...")